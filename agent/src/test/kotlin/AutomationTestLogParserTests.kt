...
class AutomationTestLogParserTests {
    companion object {
        @JvmStatic
        fun generateTestStartedTestCases(): Collection<TestStartedMessageTestCase> = listOf(
        ...
        @JvmStatic
        fun generateTestCompletedTestCases(): Collection<TestCompletedMessageTestCase> = listOf(
    ...
    @ParameterizedTest
    @MethodSource("generateTestStartedTestCases")
    fun `should parse test started message`(testCase: TestStartedMessageTestCase) {
        // act
        val testInfo = AutomationTestLogParser.tryParseTestStarted(testCase.logMessage)

        // assert
        assertEquals(testCase.expectedTestName, testInfo?.fullName)
    }

    @ParameterizedTest
    @MethodSource("generateTestCompletedTestCases")
    fun `should parse test completed message`(testCase: TestCompletedMessageTestCase) {
        // act
        val testInfo = AutomationTestLogParser.tryParseTestCompleted(testCase.logMessage)

        // assert
        assertEquals(testCase.expectedTestName, testInfo?.fullName)
        assertEquals(testCase.expectedTestResult, testInfo?.result)
    }
}
