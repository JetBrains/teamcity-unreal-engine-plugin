...
data class Workflow(
    ...
    val onCompletion: context(UnrealBuildContext) (List<Int>) -> BuildFinishedStatus = { workflowCompleted(it) },
) {
    ...
    companion object {
        ...
        private fun workflowCompleted(commandExitCodes: List<Int>): BuildFinishedStatus {
            return if (commandExitCodes.all { it == 0 } || !build.failBuildOnExitCode) {
...
sealed interface WorkflowCreationError {
    ...
    @JvmInline
    value class CommandCreationError(override val message: String) : WorkflowCreationError

    @JvmInline
    value class ExecutionPreparationError(override val message: String) : WorkflowCreationError

    @JvmInline
    value class EngineLocationError(override val message: String) : WorkflowCreationError
}
